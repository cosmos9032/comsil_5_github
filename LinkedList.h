...
public:
	LinkedList(){ // first값과 current_size를 각각 NULL과 0으로 초기화한다.
	    first = NULL;
	    current_size = 0;
	};
	int GetSize(){return current_size;};// linkedlist의 크기를 반환한다.
...
void LinkedList<T>::Insert(T element){
    Node<T> *newnode = new Node<T>(element);//newnode에 메모리르 할당해준다.
...
bool LinkedList<T>::Delete(T & element){
    ...
    while(1){
		if(current->link==0) //link가 0을 가리킬때 까지, 즉 노드의 끝까지 루프를 돌린다.
		{
		...
		previous = current;//previous에 current를 저장ㅎ나다.
